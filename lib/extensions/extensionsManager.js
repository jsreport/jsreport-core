...
module.exports = (reporter) => {
  return {
    ...
    async init () {
      ...
      if (reporter.options.extensions) {
        this.availableExtensions = this.availableExtensions.filter((e) => reporter.options.extensions.indexOf(e.name) !== -1)
      }
    ...
    async _useMany (extensions) {
      for (const e of extensions) {
        try {
          ...
          reporter.logger.error('Error when loading extension ' + err + os.EOL + err.stack)
        }
      }
    },
    ...
    async _useOne (extension) {
      try {
        extension.options = extend(true, extension.options || {}, reporter.options[extension.name])
