  ...
  Object.defineProperty(this, 'extensions', {
    get: function () {
      if (self.usedExtensions.length) {
        return self.usedExtensions.filter(function (e) {
          return e.options.enabled !== false
        })
      }
      return self.availableExtensions.filter(function (e) {
        return e.isRegistered && e.options.enabled !== false
      })
    }
  })
}
...
ExtensionsManager.prototype._discover = function () {
  ...
  return this._findAvailableExtensions().then(function (extensions) {
    ...
    var extensionsToRegister = self.reporter.options.extensions ? self.reporter.options.extensions.slice(0) : extensions.filter(function (e) {
      return e.options.enabled !== false
    }).map(function (e) {
      return e.name
    })
...
ExtensionsManager.prototype._getExtensionDirectories = function (pathToSearch) {
  ...
  if (this.reporter.options.mode === 'jsreport-development' || this.reporter.options.extensionsLocationCache === false) {
    self.reporter.logger.info('Skipping extensions location cache when NODE_ENV=jsreport-development or when option extensionsLocationCache === false, crawling now')
    ...
    .then(function () {
      return q.nfcall(fs.readFile, self.pathToLocationCache, 'utf8')
        .then(function (content) {
          ...
          if (!cache) {
            self.reporter.logger.info('Extensions location cache doesn\'t contain entry yet, crawling')
            return utils.walkSync(pathToSearch, 'jsreport.config.js')
          }

          return q.nfcall(fs.stat, path.join(__dirname, '../../')).then(function (stat) {
            if (stat.mtime.getTime() > cache.lastSync) {
              self.reporter.logger.info('Extensions location cache contains older information, crawling')
              return utils.walkSync(pathToSearch, 'jsreport.config.js')
            }
            ...
            return q.all(cache.locations.map(function (l) {
              ...
              self.reporter.logger.info('Extensions location cache contains up to date information, skipping crawling in ' + path.join(__dirname, '../../'))
      ...
      self.reporter.logger.info('Extensions location cache not found, crawling directories')
      return utils.walkSync(pathToSearch, 'jsreport.config.js')
    })
}
