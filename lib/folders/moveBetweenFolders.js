...
async function collectEntitiesInHierarchy (reporter, entities, sourceEntity, req) {
...
module.exports = (reporter) => async ({ source, target, shouldCopy, shouldReplace }, req) => {
  ...
  if (!shouldCopy) {
    ...
    const sourceEntityItem = entitiesInHierarchy.find((e) => e._id === sourceEntity._id)
    ...
    function createUpdateReferences (record) {
      return async (newEntity) => {
        const linkedEntities = entityReferencesMap.get(record.entity)

        if (linkedEntities.length === 0) {
          return
        }

        for (const originalEntity of linkedEntities) {
          const currentNewEntity = originalEntitiesNewMap.get(originalEntity)

          if (entityRecordNewValueMap.has(currentNewEntity)) {
            const currentEntityProcessedNew = entityRecordNewValueMap.get(currentNewEntity)
            ...
            reporter.documentStore.updateReference(originalEntity.__entitySet, currentEntityUpdate, record.entitySet, undefined, newEntity.shortid)

            Object.assign(currentEntityProcessedNew, currentEntityUpdate)

            await reporter.documentStore.collection(originalEntity.__entitySet).update({
              _id: currentEntityProcessedNew._id
            }, { $set: currentEntityUpdate }, req)
          } else {
            reporter.documentStore.updateReference(originalEntity.__entitySet, currentNewEntity, record.entitySet, undefined, newEntity.shortid)
          }
        }
      }
    }
    ...
    for (const entity of entitiesInHierarchy) {
      const newEntity = {
        ...omit(entity, ['_id', 'shortid', '__entitySet', 'folder'])
      }

      if (source.id === entity._id) {
        if (target.shortid === null) {
