...
module.exports = function (inputs, callback, done) {
  ...
  const {
    ...
    initialSandbox,
    {
      name: 'templatingEngines',
      timeout: inputs.templatingEngines.timeout,
      formatError: (error, moduleName) => {
        error.message += ` To be able to require custom modules you need to add to configuration { "renderingSource": "trusted" } or enable just specific module using { templatingEngines: { allowedModules": ["${moduleName}"] }`
      },
