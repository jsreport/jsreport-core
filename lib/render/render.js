...
function beforeRender (reporter, request) {
  ...
  request.toner = {
    ...
    render: function (req, res, cb) {
      reporter.render(req, res).then(function (res) {
        cb(null, res)
      }).catch(function (err) {
        cb(err)
      })
...
function afterRender (reporter, request, response) {
  return reporter.afterRenderListeners.fire(request, response).then(function () {
    reporter.logger.info('Rendering request ' + reporter.reportCounter + ' finished in ' + (new Date() - request.startTime) + ' ms')
