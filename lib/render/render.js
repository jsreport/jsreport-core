...
async function invokeRender (reporter, request, response) {
  ...
  if (
    request.data != null &&
    sharedData.contentIsArray(request.data)
  ) {
...
module.exports = async function (reporter, req, parentReq) {
  ...
  const response = Response({ meta: {} })

  request.data = reporter.createRequestData(request.data)

  request.context.id = request.context.reportCounter = ++reportCounter
  request.context.traceId = request.context.traceId || `r${request.context.id}-${nanoid()}`
  request.context.startTimestamp = new Date().getTime()

  let workerReservation

  try {
    if (reporter.options.templatingEngines.strategy === 'worker-threads' && !parentReq) {
      workerReservation = reporter.scriptManager.reserveForTask(request.context.traceId)
      reporter.activeWorkersReservation.set(request.context.traceId, workerReservation)
    }
    ...
    if (workerReservation) {
      ...
      reporter.activeWorkersReservation.delete(request.context.traceId)
    }
