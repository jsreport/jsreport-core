...
async function beforeRender (reporter, request, response) {
  ...
  reporter.logger.info('Starting rendering request ' + reportCounter + ' (user: ' + (request.user ? request.user.username : 'null') + ')', request)
...
async function invokeRender (reporter, request, response) {
  ...
  engineRes.logs.forEach(function (m) {
    reporter.logger[m.level](m.message, {timestamp: new Date(m.timestamp), ...request})
  })
