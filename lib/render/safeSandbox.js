...
module.exports = (_sandbox, options = {}) => {
  ...
  Object.keys(propsConfig).forEach((key) => {
    readOnlyProp(vm._context, key, [], proxied, true)
  })
...
function readOnlyProp (context, prop, configured, proxied, onlyTopLevel = false) {
  const parts = prop.split('.')
  const lastPartsIndex = parts.length - 1
    ...
    if (currentContext) {
      ...
      if (
        ...
        value = new Proxy(value, {
          set: (target, prop) => {
            throw new Error(`Can't add or modify property "${prop}" to read only object "${fullPropName}" inside sandbox`)
          },
          deleteProperty: (target, prop) => {
            throw new Error(`Can't delete property "${prop}" in read only object "${fullPropName}" inside sandbox`)
          }
        })
