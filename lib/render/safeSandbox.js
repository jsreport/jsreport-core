...
function doRequire (moduleName, requirePaths = []) {
  ...
  function safeRequire (require, path) {
    try {
      ...
      if (e.code && e.code === 'MODULE_NOT_FOUND') {
        if (!searchedPaths.includes(path)) {
  ...
  let result = (
    require.main ? safeRequire(require.main.require, moduleName) : false
  ) || safeRequire(require, moduleName)
  ...
  if (!result) {
    throw new Error(`Unable to find module ${moduleName}${os.EOL}Searched paths: ${os.EOL}${searchedPaths.join(os.EOL)}${os.EOL}`)
  }
