...
const requestData = require('./util/requestData')
...
module.exports = class Reporter extends events.EventEmitter {
  constructor (options, defaults) {
    ...
    this.activeWorkersReservation = new WeakMap()
 ...
 */
  async init () {
    ...
    try {
      ...
      if (this.options.templatingEngines.strategy === 'dedicated-process') {
      ...
      this.addRequestContextMetaConfig('traceId', { sandboxReadOnly: true })
 ...
 */
  ...
  createRequestData (input) {
    return requestData.createData(input, this.options.templatingEngines.strategy === 'worker-threads')
  }

  extendRequestData (req, change) {
    return requestData.extendData(req.data, change, this.options.templatingEngines.strategy === 'worker-threads')
  }
 ...
 */
  async _initOptions () {
    ...
    this.options.templatingEngines.strategy = this.options.templatingEngines.strategy || getDefaultTaskStrategy()
  ...
  _initWinston () {
    ...
    this.logger.rewriters.push(function (level, msg, meta) {
      ...
      if (meta != null && meta.context) {
        ...
        if (meta.context.traceId != null) {
          newMeta.traceId = meta.context.traceId
        }
