 ...
 */
  _loadConfig (loadExternal = true) {
    ...
    let nfn = nconf.overrides({ store: this.options })
    ...
    const transform = (obj) => {
      ...
      if (obj.key.startsWith('extensions')) {
        const match = /.*[:_](.*)[_:].*/.exec(obj.key)
    ...
    if (loadExternal) {
      nfn = nfn.argv({ transform }).env({ separator: ':', transform }).env({ separator: '_', transform })
    }
