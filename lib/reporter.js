...
Reporter.prototype._configureWinstonTransports = function (_transports) {
  ...
  var knownOptions = ['transport', 'level', 'module', 'enabled']
  ...
  Object.keys(transports).forEach(function (transpName) {
    ...
    if (typeof transpOptions.transport !== 'string' || transpOptions.transport === '') {
      throw new Error('invalid option for transport object "' + transpName + '", option "transport" has an incorrect value, must be a string with a valid value')
    }

    if (typeof transpOptions.level !== 'string' || transpOptions.level === '') {
      throw new Error('invalid option for transport object "' + transpName + '", option "level" has an incorrect value, must be a string with a valid value')
    }
    ...
    if (knownTransports[transpOptions.transport]) {
      logger.add(knownTransports[transpOptions.transport], _.extend(_.omit(transpOptions, knownOptions), {
        name: transpName,
        level: transpOptions.level
      }))
    } else {
      if (transpOptions.module == null) {
        throw new Error('invalid option for transport object "' + transpName + '", option "transport" has an unknown transport: "' + transpOptions.transport + '"')
      }

      if (transpOptions.module !== 'string') {
        throw new Error('invalid option for transport object "' + transpName + '", option "module" has an incorrect value, must be a string with a module name')
      }
      ...
      try {
        ...
        if (e.code === 'MODULE_NOT_FOUND') {
          throw new Error(
            ...
            '" in "module" option could not be found. are you sure that you have installed it?'
          )
        }
