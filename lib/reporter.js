 ...
 */
  async _initOptions () {
    ...
    if (this.options.renderingSource && !this.options.templatingEngines.allowedModules) {
      this.options.templatingEngines.allowedModules = this.options.renderingSource === 'trusted' ? '*' : []
    }

    this.options.templatingEngines.tempDirectory = this.options.tempDirectory
    this.options.templatingEngines.tempCoreDirectory = this.options.temptempCoreDirectoryDirectory
  ...
  _initWinston () {
    ...
    this.logger.rewriters.push(function (level, msg, meta) {
      ...
      if (meta != null && meta.context) {
        ...
        return Object.assign({}, omit(meta, ['template', 'options', 'data', 'context']))
      }
