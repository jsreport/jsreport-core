...
const {
  validateDuplicatedName: validateDuplicatedNameInFolder
 ...
 */
  ...
  validateDuplicatedName (c, doc, req) {
    return validateDuplicatedNameInFolder(this, c, doc, req)
  }
 ...
 */
  ...
  getAvailableRenderTimeout (req, defaultValue) {
    ...
    if (availableTimeout <= 0) {
      throw this.createError(`Render timeout after ${timeoutLimit}ms`)
    }
  ...
  async executeScript (inputs, options, req) {
    try {
      ...
      const result = await this.scriptManager.executeAsync(inputs, optionsToUse)
      return result
    } catch (e) {
