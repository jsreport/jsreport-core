...
/**
 * Manual registration of the extension. Once calling `use` the auto discovery of extensions is turned off
...
Reporter.prototype._initOptions = function () {
  ...
  return q().then(function () {
    ...
    if (!process.env.NODE_ENV) {
      self.logger.warn('Node environment not set - using development configuration. Use options --production or --development to ensure exact configuration.')
      if (self.options.loadConfig) {
        self.logger.info('Using configuration file dev.config.json')
      }
    } else {
    ...
    self.options.tasks.nativeModules = []
  })
}
