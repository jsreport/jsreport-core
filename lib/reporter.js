...
const validateHumanReadableKey = require('./store/validateHumanReadableKey')
 ...
 */
  ...
  async checkValidEntityName (c, doc, req) {
    ...
    await validateDuplicatedName(this, c, doc, req)
 ...
 */
  ...
  async executeScript (inputs, options, req) {
    try {
      ...
      optionsToUse.timeout = this.getAvailableRenderTimeout(req, options != null ? options.timeout : undefined)
      ...
      if (optionsToUse.callbackModulePath != null) {
        const callback = require(path.join(__dirname, 'scriptCallbackModuleWrapper.js')).bind(undefined, this, req, optionsToUse.callbackModulePath)
        delete optionsToUse.callbackModulePath
        optionsToUse.callback = callback
      }

      const result = await this.scriptManager.executeAsync(inputsToUse, optionsToUse)

      debugger
      result.logs.forEach(m => {
        this.logger[m.level](m.message, { ...req, timestamp: m.timestamp })
      })

      // NOTE: just for now for back-compatibility
      result.logs = [] // delete result.logs
