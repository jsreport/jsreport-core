...
module.exports = (entitySet, provider, model, validator, encryption, transactions) => ({
    ...
    return Object.assign(fakeCursor, {
      then: (onFulfilled, onRejected) => {
        ...
        let promise = listenerPromise.then(() => {
          ...
          if (activeTran) {
            opts.transaction = activeTran
          }

          return replay(provider.find(entitySet, q, p, opts))
        })
  ...
  async insert (data, req) {
    ...
    return provider.insert(entitySet, data, opts)
  },
  ...
  async remove (q, req) {
    ...
    return provider.remove(entitySet, q, opts)
  },
