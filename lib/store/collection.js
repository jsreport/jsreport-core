...
module.exports = (entitySet, provider, model, validator, encryption) => ({
  name: entitySet,
    ...
    return Promise.all(docs.map(async (doc) => {
      ...
      for (const prop in newDoc) {
        ...
        if (model.complexTypes[propType] != null) {
          ...
          const result = await this.serialize(asArray ? newDoc[prop] : [newDoc[prop]], model.complexTypes[propType])
  ...
  async deserializeProperties (docs, customTypeDef) {
    ...
    return Promise.all(docs.map(async (doc) => {
      ...
      for (const prop in newDoc) {
        ...
        if (model.complexTypes[propType] != null) {
          const asArray = isCollection && Array.isArray(newDoc[prop])
          const result = await this.deserialize(asArray ? newDoc[prop] : [newDoc[prop]], model.complexTypes[propType])
