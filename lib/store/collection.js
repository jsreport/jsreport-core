    ...
    return docs.map((doc) => {
      ...
      for (let prop in newDoc) {
        ...
        if (propDef.type === 'Edm.Binary') {
          ...
          if (!Buffer.isBuffer(newDoc[prop]) && !newDoc[prop].length) {
            let obj = newDoc[prop]
            obj = obj.data || obj
            newDoc[prop] = Object.keys(obj).map((key) => obj[key])
          }

          // unwrap mongo style buffers
          if (newDoc[prop]._bsontype === 'Binary') {
            newDoc[prop] = newDoc[prop].buffer
          }
