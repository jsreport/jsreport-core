...
const DocumentStore = (options, validator, encryption) => {
   ...
   */
    ...
    getEntityType (entityTypeName) {
      return getType(this.model, this.model.entityTypes, entityTypeName)
    },
   ...
   */
    ...
    getComplexType (complexTypeName) {
      return getType(this.model, this.model.complexTypes, complexTypeName)
    },
...
function resolvePropDefinition (model, def) {
  ...
  if (def.type.startsWith('Edm')) {
    ...
    if (childType.startsWith('Edm')) {
      ...
      const normalizedChildType = childType.replace(model.namespace + '.', '')

      if (model.complexTypes[normalizedChildType] != null) {
        result.def = def
        result.subType = model.complexTypes[normalizedChildType]
      }
    }
  } else {
    const normalizedType = def.type.replace(model.namespace + '.', '')

    if (model.complexTypes[normalizedType] != null) {
      result.def = def
      result.subType = model.complexTypes[normalizedType]
    }
  }
...
function getType (model, container, typeName) {
  const normalizedTypeName = typeName.replace(model.namespace + '.', '')
  return container[normalizedTypeName]
}
