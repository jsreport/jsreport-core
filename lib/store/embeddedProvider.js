...
EmbeddedProvider.prototype.adaptOData = function (odataServer) {
  ...
  odataServer.model(this._model)
    .onNeDB(function (col, cb) {
      ...
      self.collections[col].beforeQuery(query).then(function () {
        cb()
      }).catch(cb)
    }).beforeUpdate(function (col, query, update, cb) {
      self.collections[col].beforeUpdate(query, update).then(function () {
        cb()
      }).catch(cb)
    }).beforeRemove(function (col, query, cb) {
      self.collections[col].beforeRemove(query).then(function () {
        cb()
      }).catch(cb)
    }).beforeInsert(function (col, doc, cb) {
      self.collections[col].beforeInsert(doc).then(function () {
        cb()
      }).catch(cb)
    })
}
...
EmbeddedCollection.prototype.load = function () {
  var filename = this._options.connectionString.inMemory ? undefined : path.join(this._options.dataDirectory, this.name)
  ...
  if (this.entitySet.humanReadableKey && !this._options.connectionString.inMemory) {
