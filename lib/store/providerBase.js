...
ProviderBase.prototype.adaptOData = function (odataServer) {
  ...
  odataServer.beforeQuery(function (col, query, cb) {
    ...
    self.collections[col].beforeFindListeners.fire(query.$filter).then(function () {
      cb()
    }).catch(cb)
  }).beforeUpdate(function (col, query, update, cb) {
    self._options.logger.debug('OData update on ' + col)
    self.collections[col].beforeUpdateListeners.fire(query, update).then(function () {
      cb()
    }).catch(cb)
  }).beforeRemove(function (col, query, cb) {
    self._options.logger.debug('OData remove from ' + col + ' ' + JSON.stringify(query))
    self.collections[col].beforeRemoveListeners.fire(query).then(function () {
      cb()
    }).catch(cb)
  }).beforeInsert(function (col, doc, cb) {
    self._options.logger.debug('OData insert into ' + col)
    self.collections[col].beforeInsertListeners.fire(doc).then(function () {
      cb()
    }).catch(cb)
  })
