...
module.exports = async function executeScript (reporter, inputs, options, req) {
  try {
    ...
    inputsToUse.strategy = reporter.options.templatingEngines.strategy
    inputsToUse.timeout = optionsToUse.timeout

    if (reporter.options.templatingEngines.strategy === 'worker-threads' && reporter.activeWorkersReservation.has(req.context.traceId)) {
      optionsToUse.taskId = req.context.traceId
    }

    const useNewApiContract = Object.prototype.hasOwnProperty.call(optionsToUse, 'callbackModulePath')
