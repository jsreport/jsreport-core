...
module.exports = () => {
  ...
  return {
    register (name, checkFn, toBufferFn, fromBufferFn) {
      return serializer.register(name, checkFn, function toBuffer (value, bufferWriter) {
        const valueToProcess = toBufferFn(value)
        serializer.toBufferInternal(valueToProcess, bufferWriter)
      }, function fromBuffer (bufferReader) {
        const value = serializer.fromBufferInternal(bufferReader)
        return fromBufferFn(value)
      })
    },
